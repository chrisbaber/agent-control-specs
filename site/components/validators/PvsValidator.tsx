...
export const PvsValidator = () => {
  const [input, setInput] = useState(JSON.stringify(sampleVerdict, null, 2));
  ...
  const downloadReport = () => {
    ...
    a.href = url;
    a.download = `pvs-validation-report-${Date.now()}.json`;
    ...
    <div className="validator-section">
      <div style={{ marginBottom: '1rem' }}>
        <p style={{ fontSize: '0.9rem', color: '#999' }}>
          Validate a <strong>Policy Verdict</strong> against the PVS-1 Schema.
        </p>
      </div>
      <textarea
        value={input}
      ...
      <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem' }}>
        ...
        >
          Validate PVS-1 Verdict
        </button>
        <button
          onClick={() => setInput(JSON.stringify(sampleVerdict, null, 2))}
          style={{
            padding: '0.5rem 1.5rem',
        ...
        <div style={{
          ...
          {result.valid ? (
            <div style={{ color: '#D4AF37', fontWeight: 'bold', display: 'flex', alignItems: 'center' }}>
              <span style={{ fontSize: '1.2rem', marginRight: '0.5rem' }}>✓</span> Success: Valid PVS-1 Verdict
            </div>
          ) : (
